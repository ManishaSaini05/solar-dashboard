...
# ══════════════════════════════════════════════════════════════
if page == "Overview":
    sel_label = (active_plant if active_plant != "All Plants"
                 else f"All {', '.join(sel_brands)} Plants")
    st.markdown(f'<div class="page-hdr"><h1>Plant Overview</h1>'
                f'<p>Showing: <b>{sel_label}</b></p></div>',
                unsafe_allow_html=True)
    ...
    if not hist.empty and "today_kwh" in hist.columns:
        hist["fetched_at"] = pd.to_datetime(hist["fetched_at"])
        hist["today_kwh"]  = pd.to_numeric(hist["today_kwh"], errors="coerce")
        # Filter DB history to active plant or all selected plants
        if active_plant != "All Plants":
            hist = hist[hist["plant_name"] == active_plant]
        elif all_sel_plants:
            hist = hist[hist["plant_name"].isin(all_sel_plants)]
        m = hist[hist["fetched_at"].dt.month == datetime.now().month]
        if not m.empty:
            v = float(m.groupby([m["fetched_at"].dt.date,"inverter_sn"])["today_kwh"].max().sum())
            monthly_mwh = max(v / 1000, daily_kwh / 1000)
    ...
    # If no DB history yet, use Solis API monthly for Solis-only selections
    if monthly_mwh == 0 and sel_brands == ["Solis"] and active_plant == "All Plants":
        try:
            from utils.solis_api import fetch_summary as _fs
            summ = _fs()
            if summ.get("monthly_mwh", 0) > 0:
...
# ══════════════════════════════════════════════════════════════
    ...
    # ── DAILY ────────────────────────────────────────────────
    if rtype == "Daily":
        # Detect brand for selected plant
        sel_brand_for_report = "Solis"
        if sel_plant != "All Plants":
            info = plant_id_map.get(sel_plant)
            if info: sel_brand_for_report = info[1]
        ...
        # Both Solis and Growatt: fetch per-day energy for the selected month
        # This gives bar chart per day (same as portal shows)
        with st.spinner(f"Fetching daily data for {sel_date.strftime('%B %Y')}…"):
            if sel_plant == "All Plants":
                # Aggregate across all Solis plants
                api_rows = []
                try:
                    from utils.solis_api import get_all_plants_daily
                    api_df_all = get_all_plants_daily(month_str_d)
                    if not api_df_all.empty:
                        grp = api_df_all.groupby("date")["energy_kwh"].sum().reset_index()
                        api_rows = [{"date": str(r["date"])[:10],
                                     "energy_kwh": r["energy_kwh"]} for _, r in grp.iterrows()]
                except Exception: pass
            else:
                api_rows = get_daily_history(sel_plant, month_str_d)

        if api_rows:
            daily_df = pd.DataFrame(api_rows)
            daily_df["date"]       = pd.to_datetime(daily_df["date"], errors="coerce")
            daily_df["energy_kwh"] = pd.to_numeric(daily_df["energy_kwh"], errors="coerce").fillna(0)
            daily_df = daily_df.dropna(subset=["date"]).sort_values("date")
            daily_df = daily_df[daily_df["energy_kwh"] > 0]  # hide zero days

            if daily_df.empty:
                st.info(f"No generation data for {sel_date.strftime('%B %Y')}.")
            else:
                sec(f"Daily Generation — {sel_date.strftime('%B %Y')} (kWh per day)")

                # Highlight selected date bar
                colors = ["#f59e0b" if d.date() == sel_date else "#0d9488"
                          for d in daily_df["date"]]
                ...
                fig.update_layout(
                    plot_bgcolor="#fff", paper_bgcolor="#fff",
                    font_family="Plus Jakarta Sans", font_color="#64748b",
                    margin=dict(l=0,r=0,t=16,b=0), height=380,
                ...
                tot = daily_df["energy_kwh"].sum()
                avg = daily_df["energy_kwh"].mean()
                mx  = daily_df["energy_kwh"].max()
                c1,c2,c3,c4 = st.columns(4)
                c1.metric("Month Total",     f"{tot:.1f} kWh")
                c2.metric("Month Total MWh", f"{tot/1000:.3f} MWh")
                c3.metric("Daily Average",   f"{avg:.1f} kWh")
                c4.metric("Best Day",        f"{mx:.1f} kWh")

                # Summary table
                sec("Daily Breakdown")
                daily_df["Date"]     = daily_df["date"].dt.strftime("%d %b %Y")
                daily_df["Yield kWh"] = daily_df["energy_kwh"].round(1)
                daily_df["Earning"]  = daily_df["energy_kwh"].apply(earn)
                st.dataframe(daily_df[["Date","Yield kWh","Earning"]],
                             use_container_width=True, hide_index=True)
        else:
            # API returned nothing — fall back to local DB power readings
            hist_df = get_history(hours=8760)
            if hist_df.empty:
                st.info("📭 No data available. The API returned no historical data "
                        "and the local DB is empty. Keep the app running to build up history.")
            else:
                hist_df["fetched_at"] = pd.to_datetime(hist_df["fetched_at"])
                hist_df["power_kw"]   = pd.to_numeric(hist_df["power_kw"],  errors="coerce")
                hist_df["today_kwh"]  = pd.to_numeric(hist_df["today_kwh"], errors="coerce")
                ...
                day = hist_df[hist_df["fetched_at"].dt.date == sel_date].sort_values("fetched_at")
                if day.empty:
                    st.info(f"No data for {sel_date}. The API returned no data "
                            f"and the local DB has no readings for this date.")
                else:
                    sec("Power Output (from local DB — 5-min readings)")
                    fig = px.line(day, x="fetched_at", y="power_kw",
                                  color="inverter_sn",
                                  color_discrete_sequence=PALETTE,
                                  labels={"fetched_at":"Time","power_kw":"Power (kW)",
                                          "inverter_sn":"Inverter"})
                    st.plotly_chart(line_chart(fig, 360), use_container_width=True)
                    st.caption("⚠️ Showing locally stored readings — API historical data unavailable.")
